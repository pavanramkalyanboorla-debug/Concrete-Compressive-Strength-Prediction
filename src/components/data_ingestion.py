...
class DataIngestion:
    ...
    def initiate_data_ingestion(self):
        ...
        try:
            raw_path = os.path.join("data", "concrete.csv")
            col_names = [
                'Cement', 'Blast_Furnace_Slag', 'Fly_Ash', 'Water',
                'Superplasticizer', 'Coarse_Aggregate', 'Fine_Aggregate',
                'Age', 'Concrete_Strength'
            ]
            # The CSV has a header row, so we skip it and supply our own names
            df = pd.read_csv(raw_path, names=col_names, header=0)
