...
uncertainty_path = os.getenv("UNCERTAINTY_PATH", os.path.join("artifacts", "uncertainty_params.pkl"))
...
try:
    ...
    logger.info("Model & preprocessor loaded")
...
try:
    ...
    logger.warning("Failed to load uncertainty params: %s", e)
...
@app.get("/")
def read_root():
    return {"message": "Concrete Compressive Strength Prediction API v2.1"}
...
def metadata():
    return {
        ...
        "uncertainty_available": uncertainty_params is not None,
...
def predict_strength(input_data: PredictionInput):
    ...
    if uncertainty_params is not None:
        ...
        lower = float(pred - 1.645 * sigma)
        upper = float(pred + 1.645 * sigma)
...
def predict_batch(inputs: List[PredictionInput]):
    ...
    sigma = uncertainty_params["cv_std"] if uncertainty_params else None
    results = []
    for p in preds:
        lower = p - 1.645 * sigma if sigma else None
        upper = p + 1.645 * sigma if sigma else None
        results.append(PredictionResponse(
            predicted_strength_mpa=float(p),
...
# ------------------------------------------------------------
# SHAP endpoint (unchanged)
# ------------------------------------------------------------
from src.shap_utils import explain_with_shap_fast
...
def get_shap_plot(
    ...
    shap_values, fig = explain_with_shap_fast(mix)
    buf = BytesIO()
